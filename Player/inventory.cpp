...
void inventory::pickup(ring OnFloor) {
    ...
    } else { //Ring array is not empty
        ring *TempArr = new ring[++NRings]; //creates a new ring array with a size of Nrings+1
        for (int i = 0; i <= NRings -
                             2; i++) { //copy pastes the values of the EqRings array to the TempArr arr, loop needed since they are not the same size
            TempArr[i] = EqRings[i];
        }
...
////////////////////////////////////////////////////////////////////////////////////////////////////
int inventory::drop(int y, int x, armour armour_arr[],shield shield_arr[],weapon weapon_arr[],ring ring_arr[]) {
        ...
        switch (choice) {
            case 1: //The weapon has been chosen
                if (EqWeapon.getString() != "NONE") {//if there is a weapon equiped
                    ...
                    // set co-ordinates of board, when dropped
                    EqWeapon.setY(y);
                    EqWeapon.setX(x);

                    cout << "weapon co-ord set" << endl;

                    int counter=0;
                    while((weapon_arr[counter].getX() != -1) && (weapon_arr[counter].getY() != -1)){
                        counter++;
                        cout << counter << endl;
                    }
                    weapon_arr[counter]=EqWeapon;
            ...
            case 2: //Armour has been chosen
                if (EqArmour.getString() != "NONE") {
                    ...
                    EqArmour.setY(y);
                    EqArmour.setX(x);
                    cout << "armour co-ord set" << endl;
                    int counter=0;
                    while((armour_arr[counter].getX() != -1) && (armour_arr[counter].getY() != -1)){
                        counter++;
                        cout << counter << endl;
                    }
                    armour_arr[counter]=EqArmour;
            ...
            case 3: //the shield has been chosen
                if (EqShield.getString() != "NONE") {
                    ...
                    int counter=0;
                    while((shield_arr[counter].getX() != -1) && (shield_arr[counter].getY() != -1)){
                        counter++;
                    }
                    shield_arr[counter]=EqShield;
            ...
            default://the ring has been chosen
                if (EqRings[0].getString() != "NONE") { // if there are rings in the inventory
                    ...
                    // set co-ordinates of board, when dropped
                    EqRings[choice-4].setY(y);
                    EqRings[choice-4].setX(x);
                    // Equiped is uptodate
                    // copy Equiped in board item array
                    //
                    int counter=0;
                    while((ring_arr[counter].getX() != -1) && (ring_arr[counter].getY() != -1)){
                        counter++;
                    }
                    ring_arr[counter]=EqRings[choice - 4];

                    DeleteRing(choice - 4); //calls the function DeleteRing() ranging from 0 to NRings-1
                    to_return = 5;
                } else {
                    cout << "You have no Rings equiped!" << endl;
                } //if there are no rings, tell the user and do nothing.
        }
